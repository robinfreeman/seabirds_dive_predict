...
for (i in 1:length(files)) {
  ...
  ts_data_d$Dive = ts_data_d$Max_depth_m = NA
  
  threshold = 2.2
  
  # Assign dive profiles based on deepest depth within 30s buffer of location
  deepest = sapply(gps_idx, function(i) max(ts_data_d$Depth[(i-15):(i+15)]))
  ...
  # Create plot
  pal <- colorNumeric(palette = "RdBu", domain = loc_data$Max_depth_m)
  ...
  m <- leaflet(data = loc_data) %>% 
    ...
    addPolylines(lng = loc_data$`location-lon`, 
                 ...
                 weight = 2,
                 opacity = .4) %>%
    addLegend(position = "bottomright", pal = pal, values = loc_data$Max_depth_m) %>%
