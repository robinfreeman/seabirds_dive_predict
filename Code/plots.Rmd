...
imm.dta.full = fread('../Results/csv/IMM_WindowComp_XVal_Metrics_Keras.csv')[,1:6] %>% dplyr::filter(`Window Size (s)`<600)
acc.dta.full = fread('../Results/csv/ACC_WindowComp_XVal_Metrics_Keras.csv')[,1:6] 
```
...
acc = fread(paste0('../Results/csv/ACC_', wdw.acc, '_xval_metrics_keras.csv'), 
            select = cols, stringsAsFactors = TRUE)
imm = fread(paste0('../Results/csv/IMM_', wdw.imm, '_xval_metrics_keras.csv'),  
            select = cols, stringsAsFactors = TRUE)
...
# threshold = 0.1
cm = fread('../Results/csv/ACC_WindowComp_XVal_Metrics_Keras.csv') %>% 
        filter(`Window Size (s)` == 2) %>%
        select("TruePos (%)", "FalsePos (%)", "FalseNeg (%)", "TrueNeg (%)")
...
```{r}
res.sum.files = list.files("../Results/csv/", 
...
```{r}
pred = fread('../Results/csv/IMM_300_xval_predictions.csv')
...
# Load predicitions
dive.preds = fread('../Results/csv/IMM_300_xval_predictions.csv', stringsAsFactors = FALSE) 
...
# Load predicitions
dive.preds = fread('../Results/csv/ACC_4_xval_predictions.csv', stringsAsFactors = FALSE) 
